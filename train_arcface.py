...
from model.loss import ArcFaceLoss, ExtractFeaturesForArcFace
...
# Training settings
parser = argparse.ArgumentParser(description="ESRT with ArcFace Loss")
...
arcface_criterion = ArcFaceLoss(s=args.arcface_scale, m=args.arcface_margin)
feature_extractor = ExtractFeaturesForArcFace()
...
try:
    ...
    run = wandb.init(
        name="ESRT-ArcFace-X{}".format(args.scale),
        # id='',  # Insert specific run id here if you want to resume
        # resume="must",  # Uncomment for resuming
        project="idls25-ESRT",
        config=config,
    )
    use_wandb = True
...
def train(epoch) -> float:
    ...
    for iteration, (lr_tensor, hr_tensor) in enumerate(training_data_loader, 1):
        try:
            ...
            # Extract features and compute SR output
            sr_features, hr_features, sr_tensor = feature_extractor(model, lr_tensor, hr_tensor)
            ...
            loss_arcface = arcface_criterion(sr_features, hr_features)
    ...
    if total_batches == 0:
        return 0.0, 0.0, 0.0
    return total_loss / total_batches, total_l1_loss / total_batches, total_arcface_loss / total_batches
...
def save_checkpoint(epoch):
    model_folder = "experiment/checkpoint_ESRT_ArcFace_x{}/".format(args.scale)
...
for epoch in range(args.start_epoch, args.nEpochs + 1):
    ...
    try:
        ...
        # Then train
        train_loss, l1_loss, arcface_loss = train(epoch)
